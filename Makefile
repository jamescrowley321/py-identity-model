...
test-integration-local:
	uv run pytest src/tests -m "integration and not node_oidc" --env-file=.env.local -v -n auto -p no:benchmark

.PHONY: test-integration-ory
test-integration-ory:
	uv run pytest src/tests -m "integration and not node_oidc" -v -n auto -p no:benchmark

.PHONY: test-integration-descope
test-integration-descope:
	@echo "Running integration tests against Descope..."
	uv run pytest src/tests -m "integration and not node_oidc" $(if $(wildcard .env.descope),--env-file=.env.descope) -v -n auto -p no:benchmark
