...
| Sender-constrained (mTLS `cnf.x5t#S256`) binding + strict audience | `core/mtls.py`, `core/token_validation_logic.py`, `sync`/`aio` `token_validation.py` | RFC 8705 §3, RFC 7519 §4.1.3 | Stolen cert-bound token replayed as a plain bearer; secondary-audience confused-deputy (R.2, RT1-F1/RT5-F18) | `security/test_sender_constraint.py` | shipped |
| DPoP `cnf.jkt` resource-server proof verification | `core/dpop.py` (new verifier) | RFC 9449 §7 | DPoP-bound token replayed against a bearer RS (R.2 / #478) | _pending #478_ | pending |
