...
- **`spec/` test fixtures are not yet present.** The shared conformance vector
  ...
  then, the fixture-driven package test suites skip cleanly via a per-package
  `TestMain` guard (and the conformance suite via an in-test guard), and
  `go build ./...`, `go vet ./...`, and `go test ./...` are all green. Once
  `spec/` is imported the guards become no-ops and every vector executes for real.
