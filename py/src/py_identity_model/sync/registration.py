"""
Dynamic Client Registration (synchronous implementation, RFC 7591 / RFC 7592).
"""
...
def _post_registration(
    client: httpx.Client, url: str, json_body: dict, headers: dict
) -> httpx.Response:
    """POST a client registration request with retry logic."""
    return client.post(url, json=json_body, headers=headers)


@retry_with_backoff()
def _get_client(client: httpx.Client, url: str, headers: dict) -> httpx.Response:
    """GET a client configuration with retry logic."""
...
def _put_client(
    ...
    """PUT an updated client configuration with retry logic."""
    return client.put(url, json=json_body, headers=headers)


@retry_with_backoff()
def _delete_client(client: httpx.Client, url: str, headers: dict) -> httpx.Response:
...
def register_client(
    ...
    """Register a client at the registration endpoint (RFC 7591 Section 3).
    ...
    try:
        ...
        response = _post_registration(client, request.address, body, headers)
...
def read_client(
    ...
    """Read a client's current configuration (RFC 7592 Section 2.1).
    ...
    try:
        ...
        client = http_client.client if http_client else get_http_client()
        response = _get_client(client, request.address, headers)
...
def update_client(
    ...
    """Update a client's configuration (RFC 7592 Section 2.2).
    ...
    try:
        ...
        client = http_client.client if http_client else get_http_client()
        response = _put_client(client, request.address, body, headers)
...
def delete_client(
    ...
    """Deregister a client (RFC 7592 Section 2.3).
    ...
    try:
        headers = prepare_management_headers(request.registration_access_token)
        client = http_client.client if http_client else get_http_client()
        response = _delete_client(client, request.address, headers)
