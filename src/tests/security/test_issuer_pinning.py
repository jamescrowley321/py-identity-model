...
class TestChokepointRejectsUnapprovedIssuer:
    ...
    def test_default_no_allowlist_does_not_pin(self):
        ...
        # authoritative — no pinning. Decode proceeds and fails on the dummy token
        # for an unrelated reason, never the pinning error. Locks in that this
        # control does not change default behaviour.
        cfg = TokenValidationConfig(
            perform_disco=True, key={"kty": "RSA"}, algorithms=["RS256"]
        )
        with pytest.raises(TokenValidationException) as exc_info:
            decode_with_config("a.b.c", cfg, issuer=_ATTACKER)
        assert "not in the configured allowed_issuers" not in str(exc_info.value)
