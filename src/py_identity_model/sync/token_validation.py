...
def _refresh_jwks(jwks_uri: str, require_https: bool = True) -> JwksResponse:
...
def _discover_and_resolve_key(
    ...
    if kid is not None and not any(k.kid == kid for k in cached_keys):
        if should_attempt_kid_miss_refresh(
            ...
            jwks_response = _refresh_jwks(jwks_uri, require_https=require_https)
